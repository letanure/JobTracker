...
const ContactsView = {
	...
	create: () => {
		const container = h("div", { className: "contacts-container" },
			...
			// Contacts table
			h("div", { className: "contacts-table-container" },
				ContactsView.renderContactsTable()
			),
			
			// Table actions
			h("div", { className: "contacts-actions" },
				h("button", {
	...
	renderContactsTable: () => {
		...
		if (contacts.length === 0) {
			return h("div", { className: "contacts-empty" },
				h("p", { className: "contacts-empty-message" }, I18n.t("contactsView.emptyState"))
			);
		}
		
		return h("table", { className: "contacts-table" },
			h("thead", {},
				h("tr", {},
			...
			h("tbody", {},
				...contacts.map(contact => ContactsView.renderContactRow(contact))
			)
		);
	},
	...
	renderContactRow: (contact) => {
		return h("tr", { 
			...
			h("td", { className: "contact-cell contact-name-cell" },
				h("input", {
					type: "text",
					value: contact.name || "",
			...
			h("td", { className: "contact-cell contact-email-cell" },
				h("input", {
					type: "email",
					value: contact.email || "",
					className: "contact-field-input",
			...
			h("td", { className: "contact-cell contact-phone-cell" },
				h("input", {
					type: "tel",
					value: contact.phone || "",
					className: "contact-field-input",
			...
			h("td", { className: "contact-cell contact-role-cell" },
				h("input", {
					type: "text",
					value: contact.role || "",
					className: "contact-field-input",
			...
			h("td", { className: "contact-cell contact-company-cell" },
				h("span", { className: "contact-company-display" }, contact.jobCompany)
			),
			h("td", { className: "contact-cell contact-job-cell" },
				h("span", { className: "contact-job-display" }, contact.jobPosition)
			),
			h("td", { className: "contact-cell contact-actions-cell" },
				h("button", {
	...
	validateAndSave: (contact, field, value) => {
		if (field === 'name' && !value.trim()) {
			...
			ContactsView.showValidationError(contact, field, I18n.t("contactsView.validation.nameRequired"));
			return;
		}
	...
	deleteContact: (contact) => {
		const confirmMessage = I18n.t("contactsView.deleteConfirmation", { name: contact.name || "this contact" });
	...
	showValidationError: (contact, field, message) => {
		const contactRow = document.querySelector(`[data-contact-id="${contact.id}"][data-job-id="${contact.jobId}"]`);
		...
		const modal = h("div", { 
			...
			h("div", { className: "modal" },
				...
				h("div", { className: "modal-body" },
					h("div", { className: "add-contact-form" },
						h("div", { className: "contact-form-row" },
							h("div", { className: "contact-form-field" },
						...
						h("div", { className: "contact-form-row" },
							h("div", { className: "contact-form-field full-width" },
								h("label", {}, I18n.t("contactsView.jobHeader")),
								h("select", { className: "add-contact-job" },
