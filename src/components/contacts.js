...
const ContactItem = ({ contact, job }) => {
	...
	return h(
		...
		h(
			"div.contact-header",
			h("div.modal-header-content", h("span.contact-date", formatDate(contact.date))),
			h(
				"div.contact-actions.modal-actions-row",
		...
		h("div.contact-info", createContactDisplay(contact))
	);
};
...
const ContactsModal = ({ job, onClose }) => {
	...
	return h(
		...
		h(
			...
			h("div.modal-body", createContactsContent(job, sortedActiveContacts, sortedArchivedContacts))
		)
	);
};
