...
const TasksBoard = {
	...
	createColumn: (status) => {
		...
		const header = h("div", { className: "tasks-column-header" },
			h("div", { className: "tasks-column-title" },
	...
	createTaskCard: (task) => {
		...
		// Task content
		const content = h("div", { className: "tasks-task-content" },
			h("div", { className: "tasks-task-text" }, task.task || task.description)
		);
		
		// Job context information
		const jobContext = h("div", { className: "tasks-job-context" },
			h("div", { className: "tasks-job-info" },
			...
			h("div", { className: "tasks-job-phase" },
		...
		// Action icons
		const actionIcons = h("div", { className: "tasks-action-icons" },
			h("button", {
	...
	handleDrop: (e, targetStatus) => {
		...
		try {
			...
			const jobIndex = jobsData.findIndex(j => j.id === parseInt(jobId));
			if (jobIndex === -1) return;
			
			const taskIndex = jobsData[jobIndex].tasks.findIndex(t => t.id.toString() === taskId.toString());
